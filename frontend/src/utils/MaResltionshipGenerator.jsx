...
function MaFullSchemaGenerator() {
  ...
  const generateAllCode = () => {
    ...
    relationships.forEach(rel => {
      ...
      } else if (entity1HasMany && !entity2HasMany) { // One-to-Many: entity1 -> entity2
      ...
      } else if (!entity1HasMany && entity2HasMany) { // One-to-Many: entity2 -> entity1
    ...
    // Generate bridge tables for models.py
    ...
    // Generate entity models for models.py
    ...
    // Generate Marshmallow schemas for schemas.py
    entities.forEach(entity => {
      if (!entity.name) return;
      const rels = entityRelationships[entity.name] || [];
      const primaryField = entity.fields.length > 0 ? toSnakeCase(entity.fields[0].name) : 'id';
      
      schemasCode += `class ${entity.name}Schema(ma.SQLAlchemyAutoSchema):\n`;

      if (rels.length > 0) {
        schemasCode += `    # Nested Schemas for relationships\n`;
        rels.forEach(rel => {
          if (rel.type === 'one-to-many') {
            const relName = toSnakeCase(pluralize(rel.target));
            const backPop = toSnakeCase(entity.name);
            schemasCode += `    ${relName} = ma.Nested(${rel.target}Schema, many=True, exclude=('${backPop}',))\n`;
          } else if (rel.type === 'many-to-one') {
            const relName = toSnakeCase(rel.target);
            schemasCode += `    ${relName} = ma.Nested(${rel.target}Schema, only=('id', '${primaryField}'))\n`;
          } else if (rel.type === 'many-to-many') {
            const relName = toSnakeCase(pluralize(rel.target));
            const backPop = toSnakeCase(pluralize(entity.name));
            schemasCode += `    ${relName} = ma.Nested(${rel.target}Schema, many=True, exclude=('${backPop}',))\n`;
      ...
      schemasCode += `\n    class Meta:\n`;
      schemasCode += `        model = ${entity.name}\n`;
      schemasCode += `        load_instance = True\n`;
      schemasCode += `        include_fk = True\n\n`;
    });

    // Add schema instantiations
    schemasCode += `# Schema Instantiations\n`;
    entities.forEach(entity => {
        if (entity.name) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Build your entire database schema and serializers in minutes</p>
          </div>
             ...
             <div>
                 ...
                 <div className="flex justify-between items-center mb-4 sticky top-0 bg-gray-900 pb-2">
                   ...
                   <button
                     onClick={() => {
                       ...
                       alert('Models code copied to clipboard!');
                     }}
               ...
               <div className="bg-gray-900 text-blue-400 p-6 rounded-lg font-mono text-sm mb-6 max-h-96 overflow-y-auto">
                 <div className="flex justify-between items-center mb-4 sticky top-0 bg-gray-900 pb-2">
                   ...
                   <button
                     onClick={() => {
                       navigator.clipboard.writeText(generatedCode.schemas);
                       alert('Schemas code copied to clipboard!');
                     }}
                     className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded font-bold text-xs"
                   >
                     📋 Copy Schemas
                   </button>
                 </div>
               ...
               <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-4 mb-6">
                 ...
                 <ol className="list-decimal list-inside space-y-1 text-blue-800">
                   <li>Copy the **models** code into your <code className="bg-blue-200 px-2 py-1 rounded">app/models.py</code> file.</li>
                   <li>Copy the **schemas** code into a new <code className="bg-blue-200 px-2 py-1 rounded">app/schemas.py</code> file.</li>
                   <li>Ensure you have a <code className="bg-blue-200 px-2 py-1 rounded">app/extensions.py</code> with Marshmallow initialized (`ma = Marshmallow()`).</li>
...
export default MaFullSchemaGenerator;